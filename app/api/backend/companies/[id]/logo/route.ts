...
export async function POST(
    ...
    try {
        ...
        const response = await fetch(`${BACKEND_BASE_URL}/api/companies/${id}/logo`, {
            method: "POST",
            headers: {
        ...
        console.error("POST company logo proxy error:", error);
        return NextResponse.json({ error: "proxy_logo_upload_failed" }, { status: 500 });
    }
}